...
*/
function makeSmartPerson(thisName) {
  let object = {
    name: thisName,
...
*/
function getCarInfoByIndex(inventory, desiredIndex) {
  let carByIndex = inventory[desiredIndex];
  return `The car is a ${carByIndex.car_make} ${carByIndex.car_model}`
}
...
function getLastCarInfo(inventory) {
  const lastCarID = inventory.length - 1;
...
function getGermanCars(inventory) {
  ...
  for(var i = 0; i < inventory.length; i++){
    let germanCheck = inventory[i].car_make;
    if(
      germanCheck === "Audi" ||
      germanCheck === "Mercedes-Benz" ||
      germanCheck === "Volkswagen" ||
      germanCheck === "BMW")
      {
